...
def adjust_volume(pcm_data, volume=None):
    ...
    max_volume_factor = calculate_safe_gain(data)  # 计算一个安全的最大增益
    volume_factor = min(volume / 50., max_volume_factor)
...
class TTS:
    ...
    def _init_local(self):
        self.provider = "本地"
        self.tts = localTTS(self.conn, self.config)
        print("use local tts")

    def _init_bailian(self):
        self.provider = "百炼"
        self.tts = aliTTS(self.conn, self.config)
        print("use ali tts")
    ...
    def set_provider(self, provider):
        ...
        self.config["tts"]["provider"] = provider
